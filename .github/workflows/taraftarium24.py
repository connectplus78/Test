...
def scrape_all_channels(page):
    ...
    try:
        ...
        try:
            ...
            print("❌ Kanal elemanları zaman aşımından dolayı bulunamadı, fallback yapılacak.")
            return []

        channel_elements = page.query_selector_all(mac_item_selector)
        for element in channel_elements:
            # 1. Takım / Kanal İsmini Al
            ...
            # 2. Saat Bilgisini Al (Sitede yaygın kullanılan class'lar taranır)
            match_time = ""
            for time_class in [".saat", ".time", ".mac-saat", ".skor"]:
                time_element = element.query_selector(time_class)
                if time_element:
                    text_content = time_element.inner_text().strip()
                    if re.match(r'^\d{2}:\d{2}$', text_content):
                        match_time = text_content
                        break
            
            # Eğer saat bilgisi bulunduysa ismin başına ekle: "[20:00] Takım A - Takım B"
            if match_time:
                channel_name_clean = f"[{match_time}] {channel_name_clean}"
...
def main():
    with sync_playwright() as p:
        ...
        if not default_event_url:
            ...
            channels = [{'name': 'Varsayıvalan Kanal', 'id': default_stream_id or 'androstreamlivebs1'}]
        else:
        ...
        m3u_header_lines = [
            ...
            f"# OLUŞTURULMA ZAMANI: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}"
        ]
        ...
        if m3u_content:
            with open(output_filename, "w", encoding="utf-8") as f:
                ...
                f.write("\n".join(m3u_content))
            print(f"\n📂 {len(channels)} kanal '{output_filename}' dosyasına başarıyla kaydedildi.")
        else:
